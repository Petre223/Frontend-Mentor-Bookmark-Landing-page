      ...
      <div>
        <ul className='flex justify-center flex-col text-center my-10 md:flex-row'>
          {tabs.map((tab, index) => (
            <li 
              key={tab.id} 
              className='border-t border-slate-300 last:border-b md:border-t-0 md:border-b'>
              <button
                onClick={() => setValue(index)}  
                className={`py-4 md:px-6 ${index === value &&
                "border-b-4 border-red-400"}`}>{tab.button}</button>
            </li>
          ))}
        </ul>

        <div className='lg:flex items-center justify-center lg:gap-16
            pt-10 max-w-7xl mx-auto'>
            ...
            <article className='text-center lg:text-left lg:flex-1'>
              <h2 className='text-4xl mt-16 mb-8 lg:text-4xl'>{title}</h2>
              <p className='mb-10'>{desc}</p>
              <button 
                className='btn-blue text-white py-2 px-4 rounded shadow-lg hover:opacity-55'>{link}</button>
            </article>
        </div>
      </div>
    </section>
